import { Constants, GuidV4Provider, ObjectIdProvider } from '@digitaldefiance/ecies-lib';
...
describe('Cross-Platform ID Size Compatibility', () => {
  ...
  it('should support 16-byte GUIDs for multi-recipient encryption', async () => {
    ...
    for await (const chunk of nodeStream.decryptStreamMultiple(decryptSource, Buffer.from(id1), keyPair1.privateKey)) {
  ...
  it('should support 16-byte GUIDs for non-streaming Multiple mode', async () => {
    ...
    const cryptoCore = new EciesCryptoCore({ curveName: 'secp256k1' }, config.ECIES);
