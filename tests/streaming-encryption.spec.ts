...
import { EmailString, MemberType, Constants } from '@digitaldefiance/ecies-lib';
...
describe('Streaming Encryption', () => {
  ...
  describe('basic streaming', () => {
    ...
    it('should handle multiple chunks', async () => {
      ...
      const encrypted: Buffer[] = [];
      for await (const chunk of stream.encryptStream(source, publicKey, { chunkSize: 1024 })) {
        encrypted.push(chunk.data);
      }
    ...
    it('should handle large data', async () => {
      ...
      const encrypted: Buffer[] = [];
      for await (const chunk of stream.encryptStream(source, publicKey, { chunkSize: 64 * 1024 })) {
        encrypted.push(chunk.data);
      }
  ...
  describe('edge cases', () => {
    ...
    it('should handle very large chunk sizes', async () => {
      ...
      const encrypted: Buffer[] = [];
      for await (const chunk of stream.encryptStream(source, publicKey, { chunkSize: 10 * 1024 * 1024 })) {
        encrypted.push(chunk.data);
      }
  ...
  describe('error handling', () => {
    it('should handle invalid public key', async () => {
      ...
      await expect(async () => {
        for await (const chunk of stream.encryptStream(source, invalidKey)) {
          // Should throw
        }
      }).rejects.toThrow();
    });
    ...
    it('should handle invalid private key during decryption', async () => {
      ...
      await expect(async () => {
        for await (const chunk of stream.decryptStream(decryptSource, invalidKey)) {
          // Should throw
        }
      }).rejects.toThrow();
    });
