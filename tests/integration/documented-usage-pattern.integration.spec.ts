...
describe('Integration: Documented Usage Pattern (Node.js)', () => {
  ...
  describe('Functional Verification', () => {
    ...
    it('should correctly use ECIES config from IConstants', () => {
      ...
      expect(ecies.config.symmetricAlgorithm).toBe(config.ECIES.SYMMETRIC.ALGORITHM);
      expect(ecies.config.symmetricKeyBits).toBe(config.ECIES.SYMMETRIC.KEY_BITS);
      expect(ecies.config.symmetricKeyMode).toBe(config.ECIES.SYMMETRIC.MODE);
      expect(ecies.config.primaryKeyDerivationPath).toBe(config.ECIES.PRIMARY_KEY_DERIVATION_PATH);
      expect(ecies.config.mnemonicStrength).toBe(config.ECIES.MNEMONIC_STRENGTH);
    });
  });
